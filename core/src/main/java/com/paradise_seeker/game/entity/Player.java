...
import com.paradise_seeker.game.render.PlayerRender;
...
public class Player extends Character implements PlayerRender,  Collidable {
	public static final int MAX_HP = 1000; // Máu tối đa
	public static final int MAX_MP = 100;  // Mana tối đa

	// Các thuộc tính của nhân vật
	public float atk = 20; // Sát thương tấn công
	public float speed = 5f; // Tốc độ di chuyển

    public PlayerSkill playerSkill1 = new PlayerSkill(); // Kỹ năng 1 của người chơi
    public PlayerSkill playerSkill2 = new PlayerSkill(); // Kỹ năng 2 của người chơi
 ...
 // Thêm biến để theo dõi NPC gần nhất
    ...
    public boolean isShielding = false; // Trạng thái đang giơ khiên
    ...
    public Player() {
    	this.bounds = new Rectangle(0, 0, 1, 1); // Khởi tạo bounds mặc định
		this.hp = MAX_HP; // Thiết lập máu tối đa
		this.mp = MAX_MP; // Thiết lập mana tối đa
		this.atk = 20; // Thiết lập sát thương tấn công
		this.speed = 5f; // Thiết lập tốc độ di chuyển
		this.x = 0; // Tọa độ x khởi đầu
		this.y = 0; // Tọa độ y khởi đầu
		PlayerSkill playerskill1 = new PlayerSkill(); // Khởi tạo kỹ năng 1
		PlayerSkill playerskill2 = new PlayerSkill(); // Khởi tạo kỹ năng 2
		loadAnimations(); // Tải tất cả animation và texture
    ...
    public Player(Rectangle bounds, float hp, float mp, float maxHp, float maxMp,  float atk, float speed, float x, float y, PlayerSkill playerSkill1, PlayerSkill playerSkill2) {
        ...
        this.playerSkill1 = playerSkill1; // Gán kỹ năng 1
        this.playerSkill2 = playerSkill2; // Gán kỹ năng 2
    	loadAnimations();
    }
 ...
 // Trong update()
   ...
   // 	mp=MAX_MP;
    ...
    public void castSkill1(float atk, float x, float y) {
    ...
    public void takeDamage(float atk2) {
        if (isShielding) {
            atk2 /= 2;
        }

        hp = Math.max(0, hp - atk2);
